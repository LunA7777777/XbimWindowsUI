...
namespace XbimXplorer
{
	...
	public partial class XplorerMainWindow
			: IXbimXplorerPluginMasterWindow
			, INotifyPropertyChanged
	{
		...
		private void SetOpenedModelFileName
		(
				string ifcFilename
		)
		{
		...
		private LogEventLevel LoggingLevel
		{
			get => Settings . Default . LoggingLevel ;
		}


		private LoggingLevelSwitch loggingLevelSwitch = new LoggingLevelSwitch
		(
				LogEventLevel . Debug
		) ;
		...
		)
		{
			...
			EvaluateXbimUiType
			(
					typeof ( IfcValidation . ValidationWindow )
				  , true
			) ;
		...
		private void RefreshRecentFiles ( )
		{
			...
			if ( Settings . Default . MRUFiles != null )
				s . AddRange
				(
						Settings . Default . MRUFiles . Cast < string > ( )
				) ;


			_mruFiles = new ObservableMruList < string >
			(
					s
				  , 4
		...
		private void DrawingControl_MeasureChangedEvent
		(
				DrawingControl3D m
			  , PolylineGeomInfo e
		)
		{
			...
			if ( e . Points . Count ( ) == 1 )
			{
				...
				var modelSpace = DrawingControl . ModelPositions . GetPointInverse
				(
						new Xbim . Common . Geometry . XbimPoint3D
						(
								vspace . Point . X
							  , vspace . Point . Y
							  , vspace . Point . Z
				...
				text
						+= $" Model space in meters: X:{modelSpace . X:0.##}, Y:{modelSpace . Y:0.##}, Z:{modelSpace . Z:0.##}" ;
			}
		...
		void XplorerMainWindow_Closing
		(
				object sender
			  , CancelEventArgs e
		)
		{
			if ( _loadFileBackgroundWorker != null
			  && _loadFileBackgroundWorker . IsBusy )
			{
				Logger . LogWarning
				(
						"Closing cancelled because of active background task."
				) ;
		...
		void XplorerMainWindow_Loaded
		(
				object sender
			  , RoutedEventArgs e
		)
		{
			var model = IfcStore . Create
			(
					null
		...
		private void TestCRedist ( )
		{
			...
			Logger . LogError
			(
					"Requisite C++ environment missing, download and install from {VCPath}"
				  , Xbim . ModelGeometry . XbimEnvironment . RedistDownloadPath ( )
			) ;
		}


		private void XplorerMainWindow_Closed
		(
				object sender
			  , EventArgs e
		)
		...
		private void OpenAcceptableExtension
		(
				object s
		...
		)
		{
			...
			try
			{
				if ( worker == null )
					throw new Exception
					(
							"Background thread could not be accessed"
					) ;
						...
						if ( model . ReferencingModel is Xbim . IO . Esent . EsentModel em )
						{
							if ( ! em . CanEdit )
							{
								...
								(
										0
									  , null
									  , "Xbim models need to be opened in write mode, if they don't have geometry. Use the View/Settings/General tab to configure."
								) ;
						...
						if ( DoMesh )
						{
							...
							catch ( Exception geomEx )
							{
								...
								(
										0
									  , newexception
									  , "Error creating geometry context of {filename}"
									  , selectedFilename
								) ;
							}
						}
							...
							//if a cancellation has been requested then don't open the resulting file
					{
						...
						catch ( Exception ex )
						{
							...
							(
									0
								  , ex
								  , "Failed to cancel open of model {filename}"
								  , selectedFilename
							) ;
						}
				...
				else
				{
					Logger . LogWarning
					(
							"Settings prevent mesh creation."
					) ;
				}
		...
		)
		{
			...
			switch ( ext )
			{
				...
				case ".xbim" :
					...
					_loadFileBackgroundWorker . RunWorkerAsync
					(
							modelFileName
					) ;
				...
				default :
					...
					Logger . LogWarning
					(
							"Extension '{extension}' has not been recognised."
						  , ext
					) ;
		...
		private void FileLoadCompleted
		(
				object s
			  , RunWorkerCompletedEventArgs args
		)
		...
		private void CommandBinding_Refresh
		(
				object sender
		...
		private void CommandBinding_SaveAs
		(
				object sender
			  , ExecutedRoutedEventArgs e
		)
		...
		private void CommandBinding_Close
		(
				object sender
			  , ExecutedRoutedEventArgs e
		)
		...
		private void CommandBinding_Open
		(
				object sender
			  , ExecutedRoutedEventArgs e
		)
		...
		private void CanExecuteIfFileOpen
		(
				object sender
		...
		private void CanExecuteIfModelNotNull
		(
				object sender
			  , CanExecuteRoutedEventArgs e
		)
		...
		private void EditFederationCmdExecuted
		(
				object sender
			  , ExecutedRoutedEventArgs e
		)
		...
		private void EditFederationCmdCanExecute
		(
				object sender
			  , CanExecuteRoutedEventArgs e
		)
		...
		private void CreateFederationCmdExecuted
		(
				object sender
			  , ExecutedRoutedEventArgs e
		)
			...
			var dlg = new OpenFileDialog
			          {
					        ...
					        , Filter = "Model Files|*.ifc;*.ifcxml;*.ifczip"
					        , // Filter files by extension 
		...
		private void FederationFromDialogbox
		(
				OpenFileDialog dlg
		)
		{
			...
			switch ( firstExtension )
			{
				case ".xbimf" :
					...
					if ( dlg . FileNames . Length > 1 )
					{
						...
						(
								"Multiple files selected, open " + dlg . FileNames [ 0 ] + "?"
				...
				case ".ifcxml" :
					      ...
					      ; i ++ )
					{
						...
						try
						{
							buildRes = temporaryReference . TryBuildAndAddTo
							(
									fedModel
							) ;
						}
						...
						var res = MessageBox . Show
						(
								fileName + " couldn't be opened." + msg + "\r\nShow this message again?"
		...
		public IPersistEntity SelectedItem
		{
			get
			{
				return ( IPersistEntity ) GetValue
				(
		...
		)
		{
			...
			if ( mw != null
			  && e . NewValue is IPersistEntity )
			{
				var label = ( IPersistEntity ) e . NewValue ;


				mw . EntityLabel . Text = label != null
						                          ? "#" + label . EntityLabel
						                          : "" ;
			}
		...
		)
		{
			...
			if ( ! _camChanged )
				DrawingControl . ClipBaseSelected
				(
						0.15
				) ;
		}
		...
		private void MenuItem_ZoomExtents
		(
				object sender
			  , RoutedEventArgs e
		)
		...
		)
		{
			...
			if ( ! string . IsNullOrWhiteSpace
			     (
					     sett . AngularDeflection . Text
			     ) )
				double . TryParse
				(
						sett . AngularDeflection . Text
					  , out _angularDeflectionOverride
				) ;


			if ( ! string . IsNullOrWhiteSpace
			     (
					     sett . Deflection . Text
			     ) )
				double . TryParse
				(
						sett . Deflection . Text
					  , out _deflectionOverride
				) ;


			if ( ! string . IsNullOrWhiteSpace
			     (
					     sett . BooleanTimeout . Text
			     ) )
		...
		private void SetDefaultModeStyler
		(
				object sender
			  , RoutedEventArgs e
		)
		...
		private void ConnectStylerFeedBack ( )
		{
			...
			{
				( ( IProgressiveLayerStyler ) DrawingControl . DefaultLayerStyler ) . ProgressChanged
						+= OnProgressChanged ;
			}
		}
		...
		private void ConfigureStyler
		(
				object sender
			  , RoutedEventArgs e
		)
		{
			var c = new SurfaceLayerStylerConfiguration
			(
					Model
			) ;
			...
			if ( DrawingControl . ExcludedTypes != null )
				c . InitialiseSettings
				(
						DrawingControl . ExcludedTypes
				) ;
			...
			DrawingControl . ReloadModel
			(
					DrawingControl3D . ModelRefreshOptions . ViewPreserveCameraPosition
			) ;
		}
		...
		)
		{
			if ( null != DrawingControl . HiddenInstances )
				DrawingControl . HiddenInstances . AddRange
				(
						DrawingControl . Selection
				) ;
			else
		...
		private void OpenStrippingWindow
		(
				object sender
			  , RoutedEventArgs e
		)
		...
		private void MenuItem_ZoomSelected
		(
				object sender
			  , RoutedEventArgs e
		)
		...
		private void StylerIfcSpacesOnly
		(
				object sender
			  , RoutedEventArgs e
		)
		{
			...
			var meta2X3 = ExpressMetaData . GetMetadata
			(
					module2X3
			) ;


			var product2X3 = meta2X3 . ExpressType
			(
					"IFCPRODUCT"
			) ;
			...
			var meta4 = ExpressMetaData . GetMetadata
			(
					module4
			) ;


			var product4 = meta4 . ExpressType
			(
					"IFCPRODUCT"
			) ;


			var tpcoll = product2X3
			           . NonAbstractSubTypes
			           . Select
			             (
					             x => x . Type
		...
		private void SetStylerBoundCorners
		(
				object sender
			  , RoutedEventArgs e
		)
		...
		)
		{
			if ( e . Key == Key . Enter )
			{
				...
				var opened = OpenOrFocusPluginWindow
				             (
						             t
				             ) as Commands . wdwCommands ;
		...
		private void SetRandomStyler
		(
				object sender
			  , RoutedEventArgs e
		)
		{
			DrawingControl . DefaultLayerStyler = new RandomColorStyler
			(
					Logger
			) ;
		...
		private void SelectionColorCycle
		(
				object sender
			  , RoutedEventArgs e
		)
